...
```{r}
dat_demo <- read_excel("G:\\내 드라이브\\dev\\new_r0_data_analysis\\vein_230213.xlsx", sheet = "male_female")
dat_demo1 <- dat_demo

dat_demo1_age <- dat_demo1 %>% 
  mutate(age_gr=cut(age, 
                    c(-Inf, 29, 39, 49, 59, Inf),
                    c('20-29', '30-39','40-49','50-59','60-69')))
glimpse(dat_demo1_age)

# 성별로 나누기
dat_demo1_age_m <- dat_demo1_age %>% 
  filter(sex=="M")

dat_demo1_age_f <- dat_demo1_age %>% 
  filter(sex=="F")

# 성별 별로 나이 그룹 확인
table(dat_demo1_age_m$age_gr)
table(dat_demo1_age_f$age_gr)
...
# 비교 검정하기  
wilcox.test(dat_demo1_age_m$age, dat_demo1_age_f$age, alternative = "two.sided")
...
dat1_clean %>%
  group_by(sex) %>%
  summarise(
    mean_deapth = mean(depth, na.rm = TRUE),
     sd_depth = sd(depth, na.rm = TRUE),
    mean_d_vert = mean(d_vertical, na.rm = TRUE),
     sd_d_vert = sd(d_vertical, na.rm = TRUE),
    mean_d_hori = mean(d_horizontal, na.rm = TRUE),
     sd_d_hori = sd(d_horizontal, na.rm = TRUE)
  )
```
...
kruskal.test(depth ~ age_gr, data = male)
kruskal.test(d_vertical ~ age_gr, data = male)
kruskal.test(d_horizontal ~ age_gr, data = male)

kruskal.test(depth ~ age_gr, data = female)
kruskalmc(female$depth, female$age_gr)

kruskal.test(d_vertical ~ age_gr, data = female)
kruskalmc(female$d_vertical, male$age_gr)

kruskal.test(d_horizontal ~ age_gr, data = female)
```
...
kruskalmc(dat1_bmi$d_vertical, dat1_bmi$bmi_gr)

kruskal.test(d_horizontal ~ bmi_gr, data = dat1_bmi)
kruskalmc(dat1_bmi$d_horizontal, dat1_bmi$bmi_gr)
```
